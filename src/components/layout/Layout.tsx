...
export default function Layout({ children, title = 'Pousada App', semPadding }: LayoutProps) {
  ...
  return (
    <div className="h-screen flex bg-gray-50 text-zinc-800 overflow-hidden relative font-sans">
      {mostrarMenu && (
        <aside
          ref={menuRef}
          className={`bg-[#5d4037] text-white w-[220px] p-6 space-y-5 fixed top-0 bottom-0 z-40 transition-transform duration-300
          ${menuAberto ? 'translate-x-0' : '-translate-x-full'} lg:translate-x-0`}
          aria-label="Menu lateral"
        >
          <h2 className="text-xl font-bold mb-6 tracking-tight">Pousada App</h2>
      ...
      <div className={`${mostrarMenu ? 'lg:ml-[220px]' : ''} flex flex-col w-full h-screen transition-all duration-300`}>
        {mostrarMenu && (
          <header className="bg-[#4e8098] text-white px-4 py-3 shadow-md flex items-center gap-4">
            <button
        ...
        <main className={`flex-1 overflow-y-auto ${semPadding ? '' : 'p-6'} bg-white`}>
          {children}
        </main>
      </div>
    </div>
